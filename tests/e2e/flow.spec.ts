...
test("응답자가 회원가입 후 설문에 응답하고 결과를 확인한다", async ({ page }) => {
  ...
  await page.locator("#name").fill("E2E테스트");
  await page.locator("#email").fill(email);
  await page.locator("#password").fill("Test1234!");
  await page.locator("#passwordConfirm").fill("Test1234!");
  await page.locator("#birthYear").fill("1998");
  await page.locator("#birthMonth").fill("3");
  await page.locator("#birthDay").fill("15");
  await page.locator("#gender").selectOption("FEMALE");
  await page.locator("#phone").fill("010-9999-8888");
  await page.getByText("[필수] 응답 결과 발송").click();
  await page.getByText("[필수] 개인 식별").click();
  // 필수 동의 체크
